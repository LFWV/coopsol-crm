...
async function navigate(route, data = null) {
    ...
    if(route === 'login') app.innerHTML = ViewLogin();
        ...
        app.innerHTML = '<div style="text-align:center; padding: 3rem; font-size: 1.2rem; color: white;">Carregando dados na Nuvem...</div>';
        ...
        if(!currentUser) return navigate('login');
        app.innerHTML = '<div style="text-align:center; padding: 3rem; font-size: 1.2rem; color: white;">Calculando comissões na Nuvem...</div>';
...
<svg viewBox="0 0 300 115" style="max-width: 100%; height: auto;" xmlns="http://www.w3.org/2000/svg">
  <path d="M 50 50 A 45 45 0 0 1 140 50 Z" fill="#ffde00" />
  <path d="M 10 45 C 70 70, 140 20, 280 50 C 290 52, 290 60, 280 60 C 140 30, 70 80, 10 55 Z" fill="#009e2f" />
  <text x="25" y="95" font-family="'Inter', -apple-system, sans-serif" font-weight="800" font-size="46" fill="#f8fafc">Coopsol</text>
  <text x="32" y="110" font-family="'Inter', -apple-system, sans-serif" font-weight="500" font-size="12" fill="#94a3b8" letter-spacing="1">A Cooperativa do Sol</text>
</svg>`;
...
<div class="sidebar">
    <div class="brand" style="margin-bottom: 2rem; display: block; max-width: 200px;">
        ${CoopsolLogo()}
    </div>
...
const ViewDashboard = async () => {
    const isAdmin = currentUser.email === 'admin@coopsol.com';
        ...
        <div class="main-content">
            <div class="header">
                ...
                <div style="display: flex; gap: 1rem;">
                    <button class="btn btn-outline" style="border-color: rgba(255,255,255,0.2); color: white; padding: 0.5rem 1rem;" onclick="navigate('quickSim')"><span class="material-icons" style="font-size: 1.1rem; vertical-align: middle; margin-right: 0.4rem;">calculate</span> Rápida</button>
...
window.handleLogin = (e) => {
    ...
    if(!email.toLowerCase().endsWith('@coopsol.com')) {
        alert('Acesso restrito: Apenas e-mails corporativos (@coopsol.com) são permitidos na plataforma.');
        return;
    }
...
window.handleRegister = (e) => {
    ...
    if(!email.toLowerCase().endsWith('@coopsol.com')) {
        alert('Cadastro bloqueado: Somente e-mails finalizados em @coopsol.com podem se registrar.');
        return;
    }
